<x-app-layout>
    ...
    <section class="bg-gray-50 p-3 sm:p-5 md:pl-64 min-h-screen">
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12 space-y-5 py-8">
            <h1 class="text-gray-700 text-4xl font-semibold">Daftar Buku Dipinjam</h1>
                        ...
                        <tbody>
                            ...
                            @foreach ($borrow as $data)
                                ...
                                <tr class="border-b border-gray-300">
                                    ...
                                    <td class="px-4 py-3"><button data-modal-target="modal-{{ $data->id }}" data-modal-toggle="modal-{{ $data->id }}"><span class="px-2 py-1 rounded-md {{ $data->status === 'borrowed' ? 'bg-yellow-200 text-yellow-700' : 'bg-green-200 text-green-700'}}">{{ Str::ucfirst($data->status) }}</span></button></td>
                                ...
                                <div id="modal-{{$data->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                                        ...
                                        <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
                                            ...
                                            <?php
                                                $status = $data->status == 'borrowed' ? 'available' : 'borrowed';
                                            ?>

                                            <form action="{{ route('books.status', $data->id) }}" enctype="multipart/form-data" method="POST">
                                                @csrf
                                                @method('PUT')

                                                <div class="w-full mb-3">
                                                    <label for="status" class="block mb-2 text-sm font-medium text-gray-900">Status</label>
                                                    <select name="status" id="status" class="w-full rounded-md border-gray-300 focus:ring-teal-600">
                                                        <option value="{{ $data->status }}">{{ Str::ucfirst($data->status) }}</option>
                                                        <option value="{{ $status }}">{{ Str::ucfirst($status) }}</option>
                                                    </select>
                                                </div>

                                                <div class="w-full">
                                                    <label for="status" class="block mb-2 text-sm font-medium text-gray-900">Perpanjang Tanggal</label>
                                                    <input type="date" name="tanggal_kembali" class="w-full rounded-md border-gray-300 focus:ring-teal-600" value="{{ $data->tanggal_kembali }}">
                                                </div>
                                                    
                                                <button type="submit" class="w-full px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-white bg-teal-700 rounded-lg focus:ring-4 focus:ring-teal-200 dark:focus:ring-teal-900 hover:bg-teal-800">
                                                    Ganti
                                                </button>
                                            </form>
                                    </div>
